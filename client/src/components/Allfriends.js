...
export default function Friends({ user, userFriends }) {
  ...
  const AllFriendList = () => {
    return (
      <Link to={`/profile/${user.username}`}>
        <li class="flex space-x-2  mx-2 md:mx-3 lg:mx-5 items-center py-2 hover:bg-green-100 rounded px-1">
          <img
            src={
              user.profilePicture
                ? user.profilePicture
            ...
            alt="freind1"
            class="h-7 w-7 md:h-9 md:w-9 rounded-full"
          ></img>
  ...
  return userFriends ? <UserFriends /> : <AllFriendList />;
}
